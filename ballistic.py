...
slider_bullet_speed = slider( bind=speedBulletChange, min=50, max=1000, value=vBullet, length=250, step=50)
...
if(mode == 0):
    ...
    slider_length_speed = slider( bind=speedBulletChange, min=0, max=10, value=vBullet, length=250, step=.5)
    
    def lenChange(s):
        global L
        L = s.value
        length_text.text = f"{s.value:.1f} m/s"
