...
def my_action(b):
    ...
    if(b.text == "2D"):
        ...
        bulletAngle = 90
        slider_length = slider( bind=lenChange, min=45, max=135, value=bulletAngle, length=250, step=5)
...
def reset():
    ...
    global gr = graph(align='left', title="<b> Energy of Pendulum vs Time<b>", ytitle="Energy [J]", xtitle="Time [s]", xmin=0,ymin=0, width=400, height=280)
    global my_curve = gcurve(color=color.red, label="Kinetic Energy")
    global my_curve2 = gcurve(color=color.blue, label="Potential Energy")
    
    global ball = sphere(pos=vector(L*cos(theta),-L*sin(theta)+L,0), radius = 0.25, color = color.orange)
    global rod = cylinder(pos=vec(0, transitionY+L/2, 0), axis=ball.pos-vec(0, transitionY+L/2, 0), color=color.orange, radius = 0.03, length = L)
    ...
    if(running):
        if(mode==0):
            ...
            if(bulletShot):
                ...
                PE = (mBall+mBullet)*g*(pBall.y -ball.radius- (pivot.y - L))
